...
export abstract class WebUtils {
    ...
    async clickAndGetNetworkDataJson(element: Locator, requestPath: string, statusCode: number): Promise<object>{
        const [response] = await Promise.all([
            this.page.waitForResponse(response =>
                response.status() == statusCode && response.url() == requestPath),
            element.click()
        ])
